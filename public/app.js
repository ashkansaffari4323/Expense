...
$('create').onclick=async()=>{if($('create').disabled)return;importCancelled=false;activeImportController=null;$('cancelCreate').disabled=false;$('cancelCreate').textContent='Cancel';const valid=rows.filter(x=>x.selectionValid&&!x.existsInCost);if(!valid.length)return setActionStatus('No valid rows.',true);if(valid.length>1200)return setActionStatus('Maximum 1,200 valid rows.',true);const logical=new Map();for(const x of valid){const k=`${x.projectId}|${x.expenseName.toLowerCase()}`;if(!logical.has(k))logical.set(k,{projectId:x.projectId,project:x.projectName,originalExpenseName:x.expenseName,rows:[]});logical.get(k).rows.push(x)}const parents=[];for(const g of logical.values()){const totalParts=Math.ceil(g.rows.length/300);for(let i=0;i<g.rows.length;i+=300){const partNumber=Math.floor(i/300)+1;parents.push({...g,partNumber,totalParts,parentName:totalParts>1?`${g.originalExpenseName} - Part ${partNumber} of ${totalParts}`:g.originalExpenseName,rows:g.rows.slice(i,i+300)})}}$('create').disabled=true;$('create').textContent='Creating...';$('progressPanel').hidden=false;lastImportResults=[];let done=0,imported=0,failed=0,parentDone=0,chunkNo=0,workers=10,retries=0;const total=valid.length,desiredChunks=Math.min(5,total),parentChunkCounts=parents.map(()=>1);let extraChunks=Math.max(0,desiredChunks-parents.length);while(extraChunks>0){let best=-1,bestSize=-1;for(let i=0;i<parents.length;i++){const size=parents[i].rows.length/parentChunkCounts[i];if(parentChunkCounts[i]<parents[i].rows.length&&size>bestSize){best=i;bestSize=size}}if(best<0)break;parentChunkCounts[best]++;extraChunks--}const parentChunks=parents.map((parent,index)=>{const count=parentChunkCounts[index],chunks=[];let offset=0;for(let c=0;c<count;c++){const remaining=parent.rows.length-offset,slots=count-c,size=Math.ceil(remaining/slots);chunks.push(parent.rows.slice(offset,offset+size));offset+=size}return chunks}),totalChunks=parentChunks.reduce((n,chunks)=>n+chunks.length,0),started=Date.now();$('elapsedTime').textContent='00:00:00';if(importTimerId)clearInterval(importTimerId);importTimerId=setInterval(()=>{$('elapsedTime').textContent=formatElapsed(Date.now()-started)},1000);const update=t=>{const pct=Math.round(done/total*100),rate=done/Math.max(1,(Date.now()-started)/1000),mins=rate?Math.max(1,Math.ceil((total-done)/rate/60)):null;$('progressText').textContent=t;$('progressPercent').textContent=`${pct}%`;$('progressBar').style.width=`${pct}%`;$('progressDetail').textContent=`${done}/${total} rows | Parent ${Math.min(parentDone+1,parents.length)}/${parents.length} | Chunk ${chunkNo}/${totalChunks} | Workers ${workers} | Retries ${retries} | Imported ${imported} | Failed ${failed}${mins?` | About ${mins} min`:''}`};update('Starting...');for(let parentIndex=0;parentIndex<parents.length;parentIndex++){const parent=parents[parentIndex];if(importCancelled)break;let begin;try{activeImportController=new AbortController();begin=await api(`/api/projects/${parent.projectId}/import/start`,{method:'POST',signal:activeImportController.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({rows:parent.rows,parentName:parent.parentName})})}catch(e){failed+=parent.rows.length;done+=parent.rows.length;parentDone++;lastImportResults.push({ok:false,project:parent.project,expenseName:parent.parentName,originalExpenseName:parent.originalExpenseName,error:e.message});continue}if(begin.skipped){done+=parent.rows.length;parentDone++;continue}let parentFailed=false;for(const chunk of parentChunks[parentIndex]){if(importCancelled){parentFailed=true;break}chunkNo++;update(`Creating ${parent.parentName}`);try{activeImportController=new AbortController();const result=await api(`/api/projects/${parent.projectId}/import/${begin.expenseId}/items`,{method:'POST',signal:activeImportController.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({rows:chunk})});workers=result.adaptiveState?.concurrency||workers;retries=result.adaptiveState?.retryCount||retries;imported+=result.itemsCreated;lastImportResults.push({ok:true,project:parent.project,expenseName:parent.parentName,originalExpenseName:parent.originalExpenseName,partNumber:parent.partNumber,totalParts:parent.totalParts,rows:chunk.map(x=>x.displayRowNumber).join(','),expenseId:begin.expenseId,result})}catch(e){if(importCancelled||e.name==='AbortError'){parentFailed=true;break}failed+=chunk.length;parentFailed=true;lastImportResults.push({ok:false,project:parent.project,expenseName:parent.parentName,originalExpenseName:parent.originalExpenseName,partNumber:parent.partNumber,totalParts:parent.totalParts,rows:chunk.map(x=>x.displayRowNumber).join(','),expenseId:begin.expenseId,error:e.message})}done+=chunk.length;update(parentFailed?'Chunk error':'Chunk complete');await new Promise(r=>setTimeout(r,0))}const requested=String(parent.rows[0].status||'draft').toLowerCase();if(!parentFailed){try{activeImportController=new AbortController();const finalResult=await api(`/api/projects/${parent.projectId}/import/${begin.expenseId}/finalize`,{method:'POST',signal:activeImportController.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({status:requested})});lastImportResults.push({ok:true,project:parent.project,expenseName:parent.parentName,originalExpenseName:parent.originalExpenseName,partNumber:parent.partNumber,totalParts:parent.totalParts,expenseId:begin.expenseId,result:finalResult})}catch(e){lastImportResults.push({ok:false,project:parent.project,expenseName:parent.parentName,expenseId:begin.expenseId,error:`Items complete, status action required: ${e.message}`});setActionStatus(`Items created, but Autodesk blocked ${requested}. Check Forma workflow.`,true)}}parentDone++;update(`Completed ${parent.parentName}`)}const elapsedMilliseconds=Date.now()-started;if(importTimerId){clearInterval(importTimerId);importTimerId=null}$('elapsedTime').textContent=formatElapsed(elapsedMilliseconds);activeImportController=null;if(importCancelled){$('progressPanel').classList.add('cancelled-status');$('progressText').textContent='Import cancelled';$('progressPercent').textContent=`${Math.round(done/total*100)}%`;setActionStatus(`Import cancelled after ${done} of ${total} rows. Already-created data was kept.`,true)}else{$('progressPanel').classList.remove('cancelled-status');$('progressText').textContent='Import complete';$('progressPercent').textContent='100%';$('progressBar').style.width='100%'}$('log').value=JSON.stringify({maximumItemsPerParent:300,totalImportChunks:totalChunks,targetChunks:5,maximumChunkRows:300,initialWorkers:10,requestSpacingMs:0,chunkPauseMs:0,retryAttempts:10,retryWaitSeconds:3,elapsedSeconds:Math.round(elapsedMilliseconds/1000),elapsedTime:formatElapsed(elapsedMilliseconds),imported,failed,retries,results:lastImportResults},null,2);$('create').disabled=false;$('create').textContent='Create expenses';$('cancelCreate').disabled=true;$('cancelCreate').textContent='Cancel'};$('exportReport').onclick=async()=>{const r=await fetch('/api/export-import-report',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({results:lastImportResults})});if(!r.ok)return;const b=await r.blob(),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download='Workday Forma Import Report.xlsx';a.click();URL.revokeObjectURL(u)};
