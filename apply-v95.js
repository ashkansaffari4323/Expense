...
for(const file of Object.values(files))fs.copyFileSync(file,`${file}.before-v95.${stamp}`);
...
// Parent maximum stays 300; each browser/server batch is 250.
server=server.replace(/if\(rows\.length>(?:100|167|250|300)\)return r\.status\(400\)\.json\(\{error:'Maximum (?:100 items per chunk|167 items per batch|250 items per batch|300 items per batch)\.'\}\)/g,"if(rows.length>250)return r.status(400).json({error:'Maximum 250 items per batch.'})");
server=server.replace(/version:'v\d+',package:'1\.0\.\d+'/,"version:'v95',package:'1.0.95'");
app=app.replace(/Math\.ceil\(p\.rows\.length\/(?:100|167|250|300)\)/g,'Math.ceil(p.rows.length/250)');
app=app.replace(/i\+=(?:100|167|250|300)/g,'i+=250');
app=app.replace(/slice\(i,i\+(?:100|167|250|300)\)/g,'slice(i,i+250)');
app=app.replace(/itemChunkSize:(?:100|167|250|300)/g,'itemChunkSize:250');
// Keep all parents for a project together.
const sortPoint="$('create').disabled=true;";
if(!app.includes('v95ProjectOrder')){
  if(!app.includes(sortPoint))throw Error('Parent queue insertion point not found');
  const code="const v95ProjectOrder=new Map();parents.forEach(p=>{if(!v95ProjectOrder.has(p.projectId))v95ProjectOrder.set(p.projectId,v95ProjectOrder.size)});parents.sort((a,b)=>v95ProjectOrder.get(a.projectId)-v95ProjectOrder.get(b.projectId)||a.partNumber-b.partNumber);";
  app=app.replace(sortPoint,code+sortPoint);
}
// Add a 15-second wait after every successful 250-row batch when another batch remains in the same parent.
app=app.replace(/await new Promise\(r=>setTimeout\(r,0\)\)/g,"if(i+250<parent.rows.length&&!importCancelled){for(let seconds=15;seconds>0;seconds--){update(`Batch complete. Waiting ${seconds} seconds before next 250-row batch`);await new Promise(resolve=>setTimeout(resolve,1000))}}else{await new Promise(resolve=>setTimeout(resolve,0))}");
// Remove earlier parent/project waits, then apply 15 seconds between parents and 25 seconds between projects.
app=app.replace(/;const v9[234]NextParent=parents\[parentDone\];if\(v9[234]NextParent&&!importCancelled\)\{const v9[234]Wait=v9[234]NextParent\.projectId===parent\.projectId\?10:25;for\(let seconds=v9[234]Wait;seconds>0;seconds--\)\{update\(v9[234]NextParent\.projectId===parent\.projectId\?`Parent complete\. Waiting \$\{seconds\} seconds before next 300-item parent`:`Project \$\{parent\.project\} complete\. Waiting \$\{seconds\} seconds before next project`\);await new Promise\(resolve=>setTimeout\(resolve,1000\)\)\}\}/g,'');
if(!app.includes('v95NextParent')){
  ...
  app=app.replace(needle,"parentDone++;update(`Completed ${parent.parentName}`);const v95NextParent=parents[parentDone];if(v95NextParent&&!importCancelled){const v95Wait=v95NextParent.projectId===parent.projectId?15:25;for(let seconds=v95Wait;seconds>0;seconds--){update(v95NextParent.projectId===parent.projectId?`Parent complete. Waiting ${seconds} seconds before next parent`:`Project ${parent.project} complete. Waiting ${seconds} seconds before next project`);await new Promise(resolve=>setTimeout(resolve,1000))}}}const elapsedMilliseconds=");
}
// Automatic login when there is no valid app session.
...
// 10 workers, no normal gap, 25 linear retries from 10 through 250 seconds.
...
const pkg=JSON.parse(fs.readFileSync(files.pkg,'utf8'));pkg.version='1.0.95';fs.writeFileSync(files.pkg,JSON.stringify(pkg));
console.log('v95 applied: 250 rows per batch, 15-second successful-batch wait.');
console.log('25 retries with waits of 10, 20 ... 250 seconds.');
console.log('Backups use suffix:',`before-v95.${stamp}`);
